...
export abstract class AbstractManagementTableComponent<T> implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        protected auth?: AuthHolderService
    ) {
    }
    ...
    public replacer1(str: string) {
        str = str.replace(/[%]/g, '\\\%');
        str = str.replace(/[_]/g, '\_');
        if (str.indexOf('\\') > -1 && str.indexOf('\\_') === -1 && str !== '\\%') {
            str = str.replace(/[\\]/g, '\\\\');
        }
        return str;
    }
    ...
    filterBy(name: string, operator: Operator, value: any) {
        ...
        const replacedValue = this.replacer1(value);
        this.filteringService.and(operator, name, replacedValue);
    }

    filterBy2(name: string, operator: Operator, value: any) {
    ...
    filterByMultiple2(name: string, values: { organizationId: number }[]) {
        this.filterBy(name, Operator.ARE_MEMBERS, values && values.map(value => value.organizationId).join(','));
    }
    ...
    openEditDialogPermission(key: any, element: any = '') {
        if (key !== '') {
            if (this.auth.hasPermission(key)) {
                if (element !== '') {
    ...
    confirmDeletion(element: T, message: string = null, params: any = null, permissionkey: any = '') {
        if (permissionkey !== '') {
            if (!this.auth.hasPermission(permissionkey)) {
                return true;
            }
        }
        const data = message ? { message: message, params: params } : null;
